...
class CamelInitProcessor {
    ...
    CamelRuntimeBuildItem createInitTask(RecorderContext recorderContext, CamelTemplate template,
        ...
        for (String property : configProvider.getPropertyNames()) {
            ...
            if (property.startsWith("integration.")) {
                properties.put(property.substring("integration.".length()),
                        configProvider.getValue(property, String.class));
            }
        }
        ...
        visitServices((name, type) -> {
            LoggerFactory.getLogger(CamelInitProcessor.class).debug("Binding camel service {} with type {}", name,
                    type);
            registry.bind(name, type, recorderContext.newInstance(type.getName()));
        });
    ...
    @BuildStep(applicationArchiveMarkers = { CamelSupport.CAMEL_SERVICE_BASE_PATH,
            CamelSupport.CAMEL_ROOT_PACKAGE_DIRECTORY })
    AdditionalBeanBuildItem createCamelProducers(RecorderContext recorderContext, CamelRuntimeBuildItem runtime,
            CamelTemplate template, BuildProducer<BeanContainerListenerBuildItem> listeners) {

        listeners.produce(new BeanContainerListenerBuildItem(template.initRuntimeInjection(runtime.getRuntime())));
    ...
    @BuildStep(applicationArchiveMarkers = { CamelSupport.CAMEL_SERVICE_BASE_PATH,
            CamelSupport.CAMEL_ROOT_PACKAGE_DIRECTORY })
    void createInitTask(BeanContainerBuildItem beanContainerBuildItem, CamelRuntimeBuildItem runtime,
    ...
    protected Stream<String> getBuildTimeRouteBuilderClasses() {
        ...
        allKnownImplementors.addAll(combinedIndexBuildItem.getIndex()
                .getAllKnownSubclasses(DotName.createSimple(RouteBuilder.class.getName())));
        allKnownImplementors.addAll(combinedIndexBuildItem.getIndex()
                .getAllKnownSubclasses(DotName.createSimple(AdviceWithRouteBuilder.class.getName())));

        return allKnownImplementors.stream().filter(CamelSupport::isConcrete).filter(CamelSupport::isPublic)
