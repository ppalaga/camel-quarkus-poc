...
class JettyProcessor {
    private static final List<Class<?>> JETTY_REFLECTIVE_CLASSES = Arrays.asList(HttpClient.class,
    ...
    SubstrateConfigBuildItem process() {
        ...
        addReflectiveClass(true, true, LoginError.class.getName(), LoginToken.class.getName(),
                NotifyForFieldsEnum.class.getName(), NotifyForOperationsEnum.class.getName(), PushTopic.class.getName(),
                QueryRecordsPushTopic.class.getName(), RestChoices.class.getName(), RestErrors.class.getName());

        return SubstrateConfigBuildItem.builder().addRuntimeInitializedClass("org.eclipse.jetty.io.ByteBufferPool")
                .addRuntimeInitializedClass("org.eclipse.jetty.util.thread.QueuedThreadPool").build();
    }
